...
export const BrowseStartups = () => {
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-150 py-12 px-4 sm:px-6 lg:px-8 bg-grid-pattern">
  ...
  >
              ...
              {industries.filter((ind) => ind !== "All").map((ind) => <option key={ind} value={ind} className="bg-slate-950">{ind}</option>)}
            </select>
          </div>
    ...
    className="w-full bg-slate-950/45 border border-white/5 text-slate-300 text-xs py-2.5 px-3 rounded-lg focus:outline-none focus:border-indigo-500/50 backdrop-blur-md"
  >
              <option value="All" className="bg-slate-950">All Funding Stages</option>
              {stages.filter((stg) => stg !== "All").map((stg) => <option key={stg} value={stg} className="bg-slate-950">{stg}</option>)}
            </select>
          </div>
    ...
    /* Result grid */
        ...
        {filtered.length === 0 ? <div className="glass-card rounded-2xl p-16 text-center max-w-lg mx-auto mt-6 shadow-xl">
            ...
            <p className="font-bold text-white text-base">No Startups Found</p>
    ...
    className="mt-5 inline-block text-xs bg-slate-800 hover:bg-slate-700 text-white font-semibold py-2 px-4 rounded-lg transition"
  >
              Reset Filters
            </button>
          </div> : <>
  ...
  >
                  <div>
                    <div className="flex gap-4">
                      <span className="text-4xl bg-slate-950 p-2.5 rounded-2xl border border-slate-800 w-14 h-14 flex items-center justify-center">
                        {startup.logo}
                      </span>
                      <div className="min-w-0">
                        <h3 className="font-display font-black text-lg text-white truncate hover:text-indigo-400">
                    ...
                    <p className="text-xs text-slate-400 mt-5 leading-relaxed line-clamp-3">
                      {startup.description}
                    </p>

                    <div className="mt-5 flex items-center gap-1.5 bg-slate-950/60 rounded-lg p-2.5 border border-slate-850">
                      <MapPin size={13} className="text-slate-500" />
                      <span className="text-[11px] text-slate-400 font-mono truncate">{startup.location}</span>
                    </div>
                  </div>

                  <div className="border-t border-slate-850 mt-6 pt-5">
                    <div className="flex justify-between items-center text-xs mb-4">
                      <div>
                        <span className="text-slate-500 block text-[10px] uppercase font-mono">Founder</span>
                        <span className="text-slate-300 font-semibold text-xs">{startup.founderName}</span>
                      </div>
    ...
    className="w-full text-center bg-slate-800 hover:bg-slate-750 text-slate-100 py-2 rounded-lg text-xs font-semibold transition flex items-center justify-center gap-1"
  >
    ...
    /* Pagination simulator UI */
            ...
            <div className="flex justify-center items-center gap-2 mt-12 pb-6">
              <button disabled className="px-3 py-1 bg-slate-900 border border-slate-850 rounded text-slate-500 text-xs">Prev</button>
              <button className="px-3 py-1 bg-primary text-white border border-primary rounded text-xs font-bold">1</button>
              <button disabled className="px-3 py-1 bg-slate-900 border border-slate-850 rounded text-slate-500 text-xs">Next</button>
            </div>
          </>}
