...
export const Home = () => {
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col relative bg-grid-pattern">
    ...
    className="inline-flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-full py-1.5 px-4 mb-6 hover:border-indigo-500/30 transition-colors"
  >
  ...
  >
    ...
    className="mt-6 text-base sm:text-lg text-slate-300 max-w-2xl leading-relaxed light:text-slate-700"
  >
  ...
  >
          ...
          <Link
    to="/startups"
    className="bg-slate-900 border border-slate-850 hover:bg-slate-850 text-slate-200 hover:text-white font-medium py-3.5 px-6 rounded-lg transition-all"
  >
            Browse Startups
          </Link>
        </motion.div>
  ...
  >
          ...
          <div className="border-b border-slate-800/80 pb-3 mb-4 mt-1 flex justify-center text-xs text-slate-500 font-mono">
            startupforge_dashboard_v1.0.tsx
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-left">
            <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-850">
              ...
              <p className="font-display font-bold text-lg text-white">Find Co-Founders</p>
              <p className="text-xs text-slate-400 mt-2">Filter candidate lists by exact required technical skills, previous exits, and location preferences.</p>
            </div>
            <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-850">
              <span className="text-[10px] uppercase font-mono text-emerald-400 font-bold block mb-1">Durable Security</span>
              <p className="font-display font-bold text-lg text-white">Equity & Terms</p>
              <p className="text-xs text-slate-400 mt-2">Formalize collaborative vesting tables directly under verified regulatory legal templates.</p>
            </div>
            <div className="bg-slate-950/80 p-4 rounded-xl border border-slate-850">
              <span className="text-[10px] uppercase font-mono text-amber-500 font-bold block mb-1">Verified Growth</span>
              <p className="font-display font-bold text-lg text-white">Launch Fast</p>
              <p className="text-xs text-slate-400 mt-2">Create startup records, build open opportunities, manage applicants with clean dashboard metrics.</p>
            </div>
          </div>
        </motion.div>
      </section>
    ...
    /* Startup Statistics */
      ...
      <section className="bg-slate-900/60 border-y border-slate-850 py-12 px-4">
        <div className="max-w-7xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          <div className="p-4">
            <p className="text-3xl sm:text-4xl font-extrabold text-white font-mono tracking-tight bg-gradient-to-r from-primary to-indigo-400 bg-clip-text text-transparent">
              420+
            </p>
            <p className="text-xs text-slate-450 uppercase tracking-widest mt-1 font-mono">Total Startups</p>
          </div>
          <div className="p-4">
            <p className="text-3xl sm:text-4xl font-extrabold text-white font-mono tracking-tight bg-gradient-to-r from-secondary to-cyan-400 bg-clip-text text-transparent">
              1,850+
            </p>
            <p className="text-xs text-slate-450 uppercase tracking-widest mt-1 font-mono">Total Founders</p>
          </div>
          <div className="p-4">
            <p className="text-3xl sm:text-4xl font-extrabold text-white font-mono tracking-tight bg-gradient-to-r from-indigo-400 to-secondary bg-clip-text text-transparent">
              3,200+
            </p>
            <p className="text-xs text-slate-450 uppercase tracking-widest mt-1 font-mono">Collaborators Joined</p>
          </div>
          <div className="p-4">
            <p className="text-3xl sm:text-4xl font-extrabold text-white font-mono tracking-tight bg-gradient-to-r from-amber-400 to-orange-400 bg-clip-text text-transparent">
              890+
            </p>
            <p className="text-xs text-slate-450 uppercase tracking-widest mt-1 font-mono">Opportunities Filled</p>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 w-full">
        <div className="flex justify-between items-end mb-10">
          <div>
            ...
            <h2 className="font-display font-extrabold text-3xl text-white mt-1">Featured Startups</h2>
          </div>
  ...
  >
              <div className="flex items-center gap-4">
                <span className="text-3xl bg-slate-800 p-2.5 rounded-lg w-12 h-12 flex items-center justify-center border border-slate-700">
                  {startup.logo}
                </span>
                <div>
                  <h3 className="font-display font-bold text-lg text-white truncate max-w-[180px]">{startup.name}</h3>
                  <div className="flex items-center gap-1.5 text-xs text-slate-400 mt-0.5">
                    <span className="bg-slate-800 font-mono px-2 py-0.2.5 rounded text-white text-[10px]">
                      {startup.industry}
                    </span>
                    <span>•</span>
              ...
              <div className="border-t border-slate-800/85 mt-auto pt-4 flex justify-between items-center text-xs">
                <div>
    ...
    className="mt-5 text-center bg-slate-800 hover:bg-slate-750 text-slate-200 hover:text-white py-2 px-4 rounded-lg text-xs font-semibold transition"
  >
                View Details
              </Link>
            </div>)}
    ...
    /* Featured Opportunities Section */
      ...
      <section className="bg-slate-900/30 border-y border-slate-900 px-4 py-20 w-full">
        <div className="max-w-7xl mx-auto">
          <div className="flex justify-between items-end mb-10">
            <div>
              <span className="text-xs font-bold font-mono uppercase tracking-widest text-secondary">Apply Now</span>
              <h2 className="font-display font-extrabold text-3xl text-white mt-1">Featured Opportunities</h2>
            </div>
  ...
  >
                <div>
                  ...
                  <h3 className="font-display font-bold text-white text-base mt-3.5 leading-tight">
                    {opp.title}
                  </h3>
                  <p className="text-xs text-slate-400 mt-1 hover:underline">
    ...
    /* Skills tags */
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-4">
                    {opp.skills.map((skill, idx) => <span key={idx} className="bg-slate-800 font-mono text-[9px] px-2 py-0.5 rounded text-slate-300">
                        {skill}
                      </span>)}
                  </div>
                </div>

                <div className="border-t border-slate-850 mt-6 pt-4 flex gap-4 items-center">
                  <div className="flex-1">
                    <p className="text-[9px] text-slate-500 uppercase font-mono">Commitment</p>
                    <p className="text-xs text-white font-medium">{opp.commitment}</p>
                  </div>
                  <Link
  ...
  >
    ...
    /* Why Join Section */
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 w-full text-center">
        ...
        <h2 className="font-display font-extrabold text-3xl sm:text-4xl text-white mt-1">Why Choose StartupForge?</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-12 text-left">
          <div className="glass-card rounded-2xl p-5 hover:border-indigo-500/30 transition">
            ...
            <h3 className="font-display font-bold text-white text-base">Elite Collaborators</h3>
          ...
          <div className="glass-card rounded-2xl p-5 hover:border-cyan-500/30 transition">
            ...
            <h3 className="font-display font-bold text-white text-base">Accelerate Launch</h3>
            <p className="text-xs text-slate-400 mt-2 leading-relaxed">
          ...
          <div className="glass-card rounded-2xl p-5 hover:border-amber-500/30 transition">
            ...
            <h3 className="font-display font-bold text-white text-base">Dynamic Matching</h3>
            <p className="text-xs text-slate-400 mt-2 leading-relaxed">
          ...
          <div className="glass-card rounded-2xl p-5 hover:border-emerald-500/30 transition">
            ...
            <h3 className="font-display font-bold text-white text-base">SaaS-Grade Dashboard</h3>
            <p className="text-xs text-slate-400 mt-2 leading-relaxed">
    ...
    /* Success Stories Carousel */
      ...
      <section className="bg-slate-900/40 border-t border-slate-900 py-20 px-4 w-full">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h2 className="font-display font-extrabold text-3xl text-white mt-1 mb-10">Shared Wins</h2>
          ...
          <div className="glass-card rounded-2xl p-8 sm:p-12 relative shadow-xl overflow-hidden">
            ...
            <p className="text-sm sm:text-base text-slate-300 italic relative leading-relaxed">
  ...
  />
              <p className="text-xs font-bold text-white mt-2 mb-0.5">{testimonials[activeTestimonial].name}</p>
              <p className="text-[10px] text-slate-500 font-mono">{testimonials[activeTestimonial].role}</p>
    ...
    className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${idx === activeTestimonial ? "bg-indigo-500 w-6" : "bg-slate-700 hover:bg-slate-600"}`}
  />)}
            </div>
          </div>
        </div>
    ...
    /* CTA upgrade block */
      ...
      <section className="bg-gradient-to-tr from-primary/10 to-indigo-950/20 border-t border-slate-900 text-center py-16 px-4">
        <h2 className="font-display font-extrabold text-white text-2xl sm:text-3xl max-w-md mx-auto leading-tight">
          Ready to scale your next big venture?
        </h2>
        <p className="text-slate-400 text-xs max-w-xs mx-auto mt-2 block">
